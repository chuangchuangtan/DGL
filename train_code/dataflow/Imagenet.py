...
__all__ = ['ImagenetMeta', 'Imagenet', 'ImagenetFiles']
...
class ImagenetMeta(object):
    ...
    def get_image_list(self, name, classname, dir_structure='original'):
        ...
        Args:
            ...
            dir_structure (str): same as in :meth:`Imagenet.__init__()`.
        Returns:
...
class ImagenetFiles(RNGDataFlow):
    """
    Same as :class:`Imagenet`, but produces filenames of the images instead of nparrays.
    """
    def __init__(self, dir, name, classname, meta_dir=None,
                 shuffle=None, dir_structure=None):
        """
        Same as in :class:`Imagenet`.
        """
...
class Imagenet(ImagenetFiles):
    """
    Produces uint8 Imagenet images of shape [h, w, 3(BGR)], and a label between [0, 999].
    """
    def __init__(self, dir, name, classname, meta_dir=None,
                 shuffle=None, dir_structure=None):
        """
        Args:
            dir (str): A directory containing a subdir named ``name``, where the
                original ``Imagenet_img_{name}.tar`` gets decompressed.
            name (str): 'train' or 'val' or 'test'.
        ...
        """
        super(Imagenet, self).__init__(dir, name, classname, meta_dir, shuffle, dir_structure)
...
if __name__ == '__main__':
    ...
    ds = Imagenet('/data1/maxiaochen/ImageNet/', 'train','ILSVRC12', shuffle=True)
    ds.reset_state()
