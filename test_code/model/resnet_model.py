...
def resnet_bottleneck(l, ch_out, stride, end_points):
    """
    stride_first: original resnet put stride on first conv. fb.resnet.torch put stride on second conv.
    """
    stride_first = False
    scope = tf.get_default_graph().get_name_scope()+'_'
    shortcut = l
    l = Conv2D('conv1', l, ch_out, 1, strides=stride if stride_first else 1, activation=BNReLU)
    end_points[scope + 'conv1'] = l
    l = Conv2D('conv2', l, ch_out, 3, strides=1 if stride_first else stride, activation=BNReLU)
    end_points[scope + 'conv2'] = l
    l = Conv2D('conv3', l, ch_out * 4, 1, activation=get_bn(zero_init=True))
    end_points[scope + 'conv3'] = l
    
    out = l + resnet_shortcut(shortcut, ch_out * 4, stride, activation=get_bn(zero_init=False))
    out = tf.nn.relu(out)
    end_points[scope + 'shortcut'] = out
    
    return out


def se_resnet_bottleneck(l, ch_out, stride, end_points):
    # print('se_resnet_bottleneck return None')
    # return None
    scope = tf.get_default_graph().get_name_scope()+'_'
    shortcut = l
    l = Conv2D('conv1', l, ch_out, 1, activation=BNReLU)
...
def resnet_group(name, l, block_func, features, count, stride, end_points):
    with tf.variable_scope(name):
        for i in range(0, count):
            with tf.variable_scope('block{}'.format(i)):
                l = block_func(l, features, stride if i == 0 else 1,end_points)
    return l


def resnet_backbone(image, num_blocks = [3, 4, 6, 3], group_func = resnet_group, block_func = resnet_bottleneck,classnum = 1000):
    with argscope(Conv2D, use_bias=False,
        ...
        end_points['pool0'] = l
        l = group_func('group0', l, block_func, 64, num_blocks[0], 1, end_points)
        end_points['group0'] = l
        l = group_func('group1', l, block_func, 128, num_blocks[1], 2, end_points)
        end_points['group1'] = l
        l = group_func('group2', l, block_func, 256, num_blocks[2], 1, end_points)
        end_points['group2'] = l
        l = group_func('group3', l, block_func, 512, num_blocks[3], 1, end_points)
        end_points['group3'] = l
        layer4 = l
        l = GlobalAvgPooling('gap', l)
        
        FC_name = {200:'linearcub', 1000: 'linear'}
        logits = FullyConnected(FC_name[classnum], l, 1000,
